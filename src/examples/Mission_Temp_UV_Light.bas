...
'
' Reads a temperature sensor on B.1 input, a UV sensor on B.2, a light
' sensor on C.0 every 1 second, writes to 32K EEPROM.
'
...
symbol uv_reading       = b1
symbol light_reading    = b2
...
let i = 0   ' starting address for EEPROM memory writes
...
do
   ...
   if i < MEMORY_SIZE then
      ...
      readadc LIGHT_SEN, light_reading

      gosub WriteEEPROM
      i = i + 4    'make starting address of 3 readings a power of 2 so don't wrap on EEPROM page boundary

      sertxd ("T= ",#temp_reading, "U= ", #uv_reading, "L= ", #light_reading, 13,10)  ' output reading for easy testing
...
UploadData:
   ...
   sertxd ("Reading,Temp, UV, Light", 13,10)
   ...
   do
      gosub ReadEEPROM
      i = i + 4
      sertxd (#n, ",", #temp_reading, ",", #uv_reading, ",", #light_reading, 13,10)
...
'---------------------------------------------------------
' Arguments:  i, temp_reading, uv_reading, light_reading
'---------------------------------------------------------
WriteEEPROM:
   hi2cout i, (temp_reading, uv_reading, light_reading)	
...
'---------------------------------------------------------
ReadEEPROM:
	hi2cin i, (temp_reading, uv_reading, light_reading)
return
